...
const { createFileConditionProofStore } = require('./services/conditionProofStore');
...
const rentEscrowRoutes = require('./src/routes/rentEscrowRoutes');
const utilityMonitoringRoutes = require('./src/routes/utilityMonitoringRoutes');
...
function createApp(dependencies = {}) {
  ...
  const proofService = dependencies.conditionProofService || createConditionProofService({ store: createFileConditionProofStore() });
  const depositGatekeeper = dependencies.securityDepositService || createSecurityDepositLockService();
  ...
  app.get('/', (req, res) => {
    res.json({
      ...
      status: 'Active',
      version: '1.0.0',
      contract_id: config.contracts?.defaultContractId || 'CAEGD57WVTVQSYWYB23AISBW334QO7WNA5XQ56S45GH6BP3D2AVHKUG4'
    });
  });
  ...
  app.use('/api/smartlocks', smartLockRoutes);
  app.use('/api/escrow', rentEscrowRoutes);
  app.use('/api/utility-monitoring', utilityMonitoringRoutes);
  ...
  app.post('/renewal-proposals/:proposalId/accept', requireActorAuth(actorAuthService), (req, res) => {
    try {
      ...
      res.status(200).json({ success: true, data: result.proposal, warning: result.warning });
    } catch (error) {
      res.status(error.statusCode || 500).json({ success: false, error: error.message });
    }
  });
  ...
  app.post('/leases/:leaseId/condition-proofs', async (req, res) => {
    try {
      ...
      res.status(500).json({ error: 'CONDITION_PROOF_CREATE_FAILED', message: error.message });
    }
  });

  // --- Security Deposit Routes ---
  app.post('/move-in/generate-digital-key', requireLockedSecurityDeposit({
    action: 'Generate Digital Key',
    service: depositGatekeeper,
  }), (req, res) => {
    res.status(200).json({ success: true, message: 'Security deposit verified. Digital key generation is authorized.', verification: req.securityDepositVerification });
  });
  ...
  app.get('/api/price-feed', async (req, res) => {
    try {
      ...
      const rates = await getUSDCToFiatRates(currencies ? currencies.split(',') : ['ngn', 'eur', 'usd']);
  ...
  app.post('/api/images/optimize', upload.single('image'), async (req, res) => {
    try {
      ...
      const outputPath = path.join(uploadDir, 'optimized', `${Date.now()}_optimized.webp`);
      if (!fs.existsSync(path.dirname(outputPath))) fs.mkdirSync(path.dirname(outputPath), { recursive: true });
      await sharp(req.file.path).resize(parseInt(width), parseInt(height), { fit: 'cover' }).toFormat('webp', { quality: 80 }).toFile(outputPath);
      res.json({ success: true, path: `/uploads/optimized/${path.basename(outputPath)}` });
    } catch (error) {
...
if (require.main === module) {
  ...
  const initServices = async () => {
    ...
    try {
      ...
      console.warn('AssetMetadataService failed to initialize (Postgres might be down):', e.message);
    }
  };
  ...
  initServices().finally(() => {
    app.listen(port, () => {
      ...
      if (config.sanctions?.screeningEnabled !== false) {
        const sanctionsWorker = new SanctionsListScreeningWorker(config.sanctions);
        sanctionsWorker.initialize().then(() => {
          sanctionsWorker.start();
          console.log('Sanctions screening worker started');
        }).catch(err => {
          console.warn('Sanctions screening worker failed to initialize:', err.message);
        });
      }
    });
  });
